	...
	if (tokRes.statusCode >= 200 && tokRes.statusCode < 300) {
		...
		if (body.id_token) {
			...
			if (jose.jws.JWS.verify(body.id_token, pubKey, [rsaKey.alg])) {
				...
				// make sure the issuer matches that of our authorization server,
				if (payload.iss == 'http://localhost:9001/') {
					console.log('issuer OK');

					// Make sure our client is in the audience list
					if ((Array.isArray(payload.aud) && __.contains(payload.aud, client.client_id)) || 
						...
						// Then we’ll make sure that the issued at timestamp make sense.
						if (payload.iat <= now) {
							console.log('issued-at OK');
							// and the expiration timestamp make sense.
							if (payload.exp >= now) {
