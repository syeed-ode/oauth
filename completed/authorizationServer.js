		...
		if (code) {
			...
			if (code.request.client_id == clientId) {
				...
				if (__.contains(code.scope, 'openid') && code.user) {
					// Next we’ll create a header for our ID token
					var header = { 'typ': 'JWT', 'alg': rsaKey.alg, 'kid': rsaKey.kid };

					// and add all of the fields required for the payload (token).
					var ipayload = {
						// First we set our authorization server as the issuer
						// 		And add the subject identifier of the user
						//   	These two fields together give a globally unique
						// 		identifier for the user.
						iss: 'http://localhost:9001/', sub: code.user.sub,

						// Next, we set the client ID of the requesting client
						// 		to the audience of the token
						aud: client.client_id,

						// Finally, we’ll timestamp the token and set an
						//  	expiration of five minutes in the future.
						// 		This is more than enough time for an ID token
						//  	to be processed and tied to a user session at an RP
						iat: Math.floor(Date.now() / 1000), exp: Math.floor(Date.now() / 1000) + (5 * 60)
					};

					// Add in the 'nonce' value, if the client sent it on the original
					//   	request to the authorization endpoint. Like the 'state'
					//  	parameter, but closes a different XSS attack vector
					if (code.request.nonce) {
						ipayload.nonce = code.request.nonce;
					}

					// Sign it with the server’s key and serialize it as a (JSON Web
					//   	Token) JWT.
					...
					// issue it alongside the access token by modifying the existing
					//  	token response.
